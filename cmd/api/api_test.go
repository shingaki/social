...
func TestRateLimiterMiddleware(t *testing.T) {
	...
	app.rateLimiter = ratelimiter.NewFixedWindowLimiter(cfg.rateLimiter.RequestsPerTimeFrame, cfg.rateLimiter.TimeFrame)

	log.Printf("rate limiter middleware enabled: %v", cfg.rateLimiter.Enabled)
	ts := httptest.NewServer(app.mount())
	log.Println("after httptest.NewServer")
